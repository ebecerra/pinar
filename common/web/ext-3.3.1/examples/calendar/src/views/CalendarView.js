...
Ext.calendar.CalendarView = Ext.extend(Ext.BoxComponent, {
    ...
    setStartDate: function(start, refresh) {
        ...
        this.setViewBounds(start);
            var appCalendar = Ext.getCmp('app-calendar');
            if (appCalendar && appCalendar.pacienteData) {
                var d = new Date(this.startDate.getFullYear(), this.startDate.getMonth(), 1);
                this.store.removeAll();
                this.store.load({
                    params: {
                        fecha_inicio : d.add(Date.HOUR, 0).add(Date.MINUTE, 1).format('n-j-Y H:i'),
                        fecha_fin: this.viewEnd.add(Date.HOUR, 23).add(Date.MINUTE, 59).format('n-j-Y H:i'),
                        id_paciente: appCalendar.pacienteData.id_paciente
                    }
                });
            }
            else{
              //alert("setStartDate 2 "+ start );
                var d = new Date(this.startDate.getFullYear(), this.startDate.getMonth(), 1);
                this.store.removeAll();
                this.store.load({
                    params: {
