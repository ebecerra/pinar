package com.befasoft.common.model.manager;

import com.befasoft.common.model.EntityBean;
import com.befasoft.common.model.appbs.APPBS_USUARIOS_LOG_LEGEND;
import com.befasoft.common.tools.HibernateUtil;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

/**
 * Created by Devtools.
 * Date: 31/10/2011
 */
public class APPBS_USUARIOS_LOG_LEGEND_MANAGER extends BeanManager {

    private static String BEAN_NAME ="APPBS_USUARIOS_LOG_LEGEND";

    /**
     * Lista todos los elementos
     */
    public static List listAll() {
        return listAll(BEAN_NAME);
    }

    /**
     * Lista los elementos que cumplan con el criterio de filtrado
     */
    public static List findByFilter(Map filters) {
        return findByFilter(BEAN_NAME, filters);
    }

    public static int findCountByFilter(Map<String, String> filter) {
        return findCountByFilter(HibernateUtil.currentSession(), BEAN_NAME, filter);
    }

    /**
     * Lista un elemento que cumplan con el criterio de filtrado
     */
    public static APPBS_USUARIOS_LOG_LEGEND findFirstByFilter(Map filters) {
        return  (APPBS_USUARIOS_LOG_LEGEND) findFirstByFilter(BEAN_NAME, filters);
    }

    /**
     * Busca por el campo llave
     * "id_tipo"
     */
    public static APPBS_USUARIOS_LOG_LEGEND findByKey(Long id_tipo ) {
        Map filters = new HashMap();
        filters.put("id_tipo", id_tipo);

        return findFirstByFilter(filters);
    }

    public static APPBS_USUARIOS_LOG_LEGEND findByKey(Map keyValues) {
        EntityBean bean = new APPBS_USUARIOS_LOG_LEGEND();
        return (APPBS_USUARIOS_LOG_LEGEND) findByKey(bean, keyValues);
    }

}
