    ...
    private void bean(Object object) throws JSONException {
        ...
        try {
            ...
            for (int i = 0; i < props.length; ++i) {
                ...
                //System.out.println("  - prop = " + accessor);
                ...
                if (baseAccessor != null) {
                    ...
                    // Sino esta la anotacion se busca si esta en el campo.
                    if (json == null) {
                        try {
                            Field field = clazz.getDeclaredField(name);
                            json = field.getAnnotation(JSON.class);
                        }
                        catch (Throwable e) {
                        }
                    }
                    if (json != null) {
                        if (!json.serialize()) {
                            //System.out.println("Se excluye del JSON "+baseAccessor);
                            continue;
                        }
...
//        if (object.getClass().getName().indexOf("ESCUELA") >= 0) {
//            System.out.println("this.buf = " + this.buf);
//        }
        this.add("}");
    }
    ...
    private boolean shouldExcludeProperty(Class clazz, PropertyDescriptor prop)
        ...
        if (Collection.class.isAssignableFrom(clazz) || propClazz.getName().startsWith("org.hibernate") || name.equals("class") || name.equals("declaringClass")
